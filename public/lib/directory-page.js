(function exposeDirectoryPage(root, factory) {
  ...
  function artistCardMarkup(artist, { escapeHtml, formatGigDate, initials }) {
      ...
      while (active < 2 && queue.length) {
        ...
        entityInfo(task.type, task.name, task.city).then((info) => {
          ...
          if (info.image && !task.card.querySelector('.entity-card-image img')) {
            const image = document.createElement('img'); image.alt = ''; image.loading = 'lazy'; image.decoding = 'async'; image.style.objectPosition = info.imagePosition || 'center';
  ...
  function createController({ page, window, getShows, getRemoteShows, loadMetadata, directoryUi, escapeHtml, formatGigDate, hydrator, elements }) {
    async function render() {
      ...
      const draw = () => {
        ...
        controls.grid.innerHTML = visible.map((entity) => config.card(entity, { escapeHtml, formatGigDate, initials: directoryUi.initials })).join('') || `<p class="empty-state entity-directory-empty">${config.empty}</p>`;
