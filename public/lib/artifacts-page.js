  ...
  function createController({ page, getShows, escapeHtml, formatGigDate, openArtifact = () => {}, elements }) {
    ...
    function render() {
      ...
      grid.innerHTML = filtered.map((item) => {
        ...
        const artifactHref = `/artifact?id=${encodeURIComponent(item.id)}`;
        return `<article class="artifact-archive-card${item.useBackgroundRemoved ? ' is-cutout' : ''}" data-artifact-id="${escapeHtml(item.id)}"><button class="artifact-archive-image" type="button" aria-label="Preview ${escapeHtml(item.caption || labels[artifactType])}"><img src="${escapeHtml(item.url || '')}" alt="${escapeHtml(item.caption || `${labels[artifactType]} from ${item.gig.artist}`)}" loading="lazy" style="${imageStyle}" />${viewCount > 1 ? `<span>${viewCount} views · flip</span>` : ''}</button><div class="artifact-archive-copy"><span class="artifact-type-badge">${escapeHtml(labels[artifactType] || labels.memorabilia)}</span><h2><a href="${artifactHref}">${escapeHtml(item.caption || item.filename || labels[artifactType] || labels.memorabilia)}</a></h2>${item.artifactNotes ? `<p>${escapeHtml(item.artifactNotes)}</p>` : ''}<a class="artifact-detail-link" href="${artifactHref}">View artifact →</a><a class="artifact-show-link" href="${showHref}"><strong>${escapeHtml(item.gig.artist)}</strong><span>${escapeHtml(item.gig.venue)} · ${escapeHtml(item.gig.city)}</span><time>${escapeHtml(formatGigDate(item.gig.date))}</time></a></div></article>`;
      }).join('');
