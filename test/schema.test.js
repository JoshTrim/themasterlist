...
describe('SQLite schema migrations', () => {
  ...
  test('creates every current subsystem table and can run repeatedly', () => {
    ...
    for (const table of ['gigs', 'gig_media', 'media_playback_clips', 'profiles', 'sessions', 'instance_identity', 'peer_instances', 'peer_sync_baselines', 'peer_sync_conflicts', 'app_settings', 'background_jobs']) assert.ok(tables.has(table), table);
  });
