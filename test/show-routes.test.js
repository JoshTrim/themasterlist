...
test('show routes create, update, list and delete without replacing track metadata', async () => {
  ...
  let body = { artist: ' Artist ', venue: ' Venue ', city: ' Brisbane ', date: '2026-01-01', songs: [{ title: 'Song', album: 'Album', info: 'keep' }], acts: [{ artist: ' Support ', role: 'Opener', songs: [{ title: 'Opening song' }] }] };
  const replies = [];
  let id = 0;
  ...
  assert.equal(replies[0].status, 201); assert.equal(replies[0].payload.artist, 'Artist'); assert.equal(replies[0].payload.acts[0].artist, 'Support');
  body = { songs: [{ title: 'Renamed' }], acts: [{ artist: 'Co-headliner', role: 'Co-headliner', songs: [] }] };
  await handle({ method: 'PATCH', account: {} }, {}, new URL('http://x/api/gigs/id-1'));
  assert.equal(replies[1].payload.songs[0].album, 'Album'); assert.equal(replies[1].payload.songs[0].info, 'keep'); assert.equal(replies[1].payload.acts[0].role, 'Co-headliner');
