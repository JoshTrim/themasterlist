...
function migrateSchema(database) {
  ...
  database.exec(`CREATE TABLE IF NOT EXISTS playlist_exports (
    id TEXT PRIMARY KEY,
    gig_id TEXT NOT NULL,
  ...
  database.exec('CREATE INDEX IF NOT EXISTS playlist_exports_gig_provider ON playlist_exports (gig_id, provider, created_at DESC)');
  ...
  database.prepare("UPDATE playlist_exports SET status = 'error', error = 'Interrupted by server restart. Start the export again to resume it.', error_code = 'interrupted', updated_at = ? WHERE status IN ('queued', 'running')").run(new Date().toISOString());
  ...
  addColumnIfMissing('gig_media', 'artifact_type', "TEXT NOT NULL DEFAULT 'memorabilia'");
  addColumnIfMissing('gig_media', 'artifact_notes', "TEXT NOT NULL DEFAULT ''");
  addColumnIfMissing('gig_media', 'artifact_crop_x', 'REAL NOT NULL DEFAULT 50');
  addColumnIfMissing('gig_media', 'artifact_crop_y', 'REAL NOT NULL DEFAULT 50');
  addColumnIfMissing('gig_media', 'artifact_zoom', 'REAL NOT NULL DEFAULT 1');
  addColumnIfMissing('gig_media', 'artifact_group_id', 'TEXT');
  addColumnIfMissing('gig_media', 'artifact_view', "TEXT NOT NULL DEFAULT 'front'");
  addColumnIfMissing('gig_media', 'artifact_view_label', "TEXT NOT NULL DEFAULT ''");
  addColumnIfMissing('gig_media', 'artifact_is_cover', 'INTEGER NOT NULL DEFAULT 0');
  database.prepare("UPDATE gig_media SET artifact_group_id = id, artifact_is_cover = 1 WHERE category = 'artifact' AND (artifact_group_id IS NULL OR artifact_group_id = '')").run();
  database.exec('CREATE INDEX IF NOT EXISTS gig_media_artifact_group ON gig_media (gig_id, artifact_group_id, sort_order, created_at)');
  ...
  addColumnIfMissing('gigs', 'acts', "TEXT NOT NULL DEFAULT '[]'");
  ...
  `);
  addColumnIfMissing('shared_shows', 'acts', "TEXT NOT NULL DEFAULT '[]'");
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS dismissed_shared_shows (
      shared_gig_id TEXT PRIMARY KEY,
  ...
  addColumnIfMissing('peer_instances', 'last_sync_at', 'TEXT');
  addColumnIfMissing('peer_instances', 'last_attempt_at', 'TEXT');
  addColumnIfMissing('peer_instances', 'last_error', 'TEXT');
  addColumnIfMissing('peer_instances', 'consecutive_failures', 'INTEGER NOT NULL DEFAULT 0');
  addColumnIfMissing('peer_instances', 'next_retry_at', 'TEXT');
