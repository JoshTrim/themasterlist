...
function createAuth({ database, env = process.env, now = () => new Date() }) {
  ...
  function currentAccount(request) {
    ...
    return database.prepare(`SELECT p.id, p.name, p.is_admin AS isAdmin FROM sessions s
      ...
      WHERE s.token_hash = ? AND s.expires_at > ? AND p.is_admin = 1`).get(tokenHash(token), now().toISOString()) || null;
  }

  function accountsConfigured() {
    return database.prepare('SELECT COUNT(*) AS count FROM profiles WHERE password_hash IS NOT NULL AND is_admin = 1').get().count > 0;
  }
