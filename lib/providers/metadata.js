...
function wikipediaArticle(value) {
  let url;
  try { url = new URL(String(value || '').trim()); }
  catch {
    const error = new Error('Enter a valid Wikipedia article URL.'); error.status = 400; throw error;
  }
  const host = url.hostname.toLowerCase().replace(/\.m\.wikipedia\.org$/, '.wikipedia.org');
  if (url.protocol !== 'https:' || !/^[a-z0-9-]+\.wikipedia\.org$/.test(host) || url.port || url.username || url.password || !url.pathname.startsWith('/wiki/')) {
    const error = new Error('Use an HTTPS Wikipedia article URL.'); error.status = 400; throw error;
  }
  let title;
  try { title = decodeURIComponent(url.pathname.slice('/wiki/'.length)).replace(/_/g, ' ').trim(); }
  catch { title = ''; }
  if (!title) { const error = new Error('Use a complete Wikipedia article URL.'); error.status = 400; throw error; }
  return { host, title, source: `https://${host}/wiki/${encodeURIComponent(title.replace(/ /g, '_'))}` };
}
...
function createMetadataProvider({ fetch: request = globalThis.fetch, googleApiKey = '', googleEngineId = '', headers = DEFAULT_HEADERS } = {}) {
  ...
  async function artistInfoFromUrl(name, sourceUrl) {
    const article = wikipediaArticle(sourceUrl);
    const response = await request(`https://${article.host}/api/rest_v1/page/summary/${encodeURIComponent(article.title.replace(/ /g, '_'))}`, { headers });
    if (!response.ok) { const error = new Error('Wikipedia could not load that artist article.'); error.status = 404; throw error; }
    const summary = await response.json();
    return {
      name, title: summary.title || name, description: summary.description || '', bio: summary.extract || '',
      image: summary.thumbnail?.source || summary.originalimage?.source || null,
      source: summary.content_urls?.desktop?.page || article.source
    };
  }
  ...
  return { artistGenre, artistInfo, artistInfoFromUrl, venueInfo, album };
}

module.exports = { DEFAULT_HEADERS, cleanMusicName, pageMetadata, wikipediaArticle, createMetadataProvider };
