...
function createSetlistFmProvider({ apiKey, fetch: request = globalThis.fetch, recordUsage = () => {}, normaliseSongs }) {
  ...
  async function searchEvent({ venueId, venueName, cityName, eventDate }) {
    if (!configured()) throw new SetlistProviderError('Add SETLIST_FM_API_KEY to .env before searching setlist.fm.', 503);
    if ((!venueId && !venueName) || !eventDate) throw new SetlistProviderError('A venue and date are required to find other performers.', 400);
    const upstream = new URL(SEARCH_ENDPOINT);
    if (venueId) upstream.searchParams.set('venueId', String(venueId).trim());
    else upstream.searchParams.set('venueName', String(venueName).trim());
    if (!venueId && cityName) upstream.searchParams.set('cityName', String(cityName).trim());
    upstream.searchParams.set('date', String(eventDate).split('-').reverse().join('-'));
    const response = await performSearch(upstream, 'search/event-setlists');
    if (response.status === 404) return { total: 0, setlists: [] };
    if (!response.ok) throw new SetlistProviderError('setlist.fm could not find other performers for this event.', response.status);
    const result = await response.json();
    const setlists = (result.setlist || []).map((setlist) => ({
      id: setlist.id,
  ...
  return { configured, search, searchEvent };
}
