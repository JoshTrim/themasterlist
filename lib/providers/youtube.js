...
function createYouTubeProvider({ requestJson }) {
  ...
  async function searchLiveVideos({ gig, song, accessToken }) {
    ...
    const embeddableIds = new Set((statusResult.items || []).filter((item) => item.status?.embeddable === true).map((item) => item.id));
...
module.exports = { eventNeedles, createYouTubeProvider };
