function createPeerSync({ database, identity, transport, findGig, normaliseRating, createHash, detectConflict, now = () => new Date().toISOString() }) {
  ...
  function mediaManifest(gig) {
    return (gig.media || []).map((item) => ({
      ...
      artifactType: item.artifactType || 'memorabilia', artifactNotes: item.artifactNotes || '',
      artifactCropX: Number(item.artifactCropX ?? 50), artifactCropY: Number(item.artifactCropY ?? 50), artifactZoom: Number(item.artifactZoom ?? 1),
      artifactGroupId: item.artifactGroupId || (item.category === 'artifact' ? item.id : null),
      artifactView: item.artifactView || 'front', artifactViewLabel: item.artifactViewLabel || '', artifactIsCover: Boolean(item.artifactIsCover),
  ...
  function ensureSharedShow(gig) {
    ...
    database.prepare('DELETE FROM dismissed_shared_shows WHERE shared_gig_id = ?').run(sharedGigId);
    database.prepare(`INSERT INTO shared_shows
      ...
      ON CONFLICT(id) DO UPDATE SET source_gig_id=COALESCE(shared_shows.source_gig_id, excluded.source_gig_id),
        ...
        setlist_fm_id=excluded.setlist_fm_id, setlist_fm_url=excluded.setlist_fm_url, songs=excluded.songs, acts=excluded.acts`).run(
      sharedGigId, gig.id, gig.artist, gig.venue, gig.city, gig.date, gig.setlistFmId || null, gig.setlistFmUrl || null,
      JSON.stringify(gig.songs || []), JSON.stringify(gig.acts || []), gig.createdAt || now()
    );
    return sharedGigId;
  }
  ...
  function localSnapshots(peerId) {
    ...
    return database.prepare('SELECT id FROM gigs ORDER BY created_at').all().map((row) => findGig(row.id)).filter((gig) =>
      ...
      const show = { artist: gig.artist, venue: gig.venue, city: gig.city, date: gig.date, setlistFmId: gig.setlistFmId, setlistFmUrl: gig.setlistFmUrl, songs: gig.songs || [], acts: gig.acts || [] };
  ...
  function applySnapshot(snapshot, originPeer) {
    ...
    if (database.prepare('SELECT 1 FROM dismissed_shared_shows WHERE shared_gig_id = ?').get(snapshot.sharedGigId)) return false;
    ...
    database.transaction(() => {
      database.prepare(`INSERT INTO shared_shows
        ...
        show.setlistFmId || null, show.setlistFmUrl || null, JSON.stringify(Array.isArray(show.songs) ? show.songs : []), JSON.stringify(Array.isArray(show.acts) ? show.acts : []), timestamp
      );
      const contribution = snapshot.contribution;
      database.prepare(`INSERT INTO shared_gig_contributions
        (shared_gig_id, instance_id, local_gig_id, participant_name, performance_rating, venue_rating, favorite, performance_notes, venue_notes, media_manifest, updated_at)
