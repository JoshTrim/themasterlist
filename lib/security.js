...
const PEER_PATHS = new Set(['/api/sync/pair', '/api/sync/hello', '/api/sync/exchange', '/api/sync/media']);
...
function validateRequestOrigin(request, pathname, trustedOrigin) {
  return requestOriginDiagnostic(request, pathname, trustedOrigin).valid;
}

function requestOriginDiagnostic(request, pathname, trustedOrigin) {
  ...
  if (rawOrigin) {
    try { receivedOrigin = new URL(rawOrigin).origin; } catch { receivedOrigin = 'invalid origin'; }
  }
  if (!MUTATING_METHODS.has(request.method) || PEER_PATHS.has(pathname)) return { valid: true, trustedOrigin, receivedOrigin, secFetchSite, reason: null };
  if (secFetchSite === 'cross-site') return { valid: false, trustedOrigin, receivedOrigin, secFetchSite, reason: 'browser marked the request as cross-site' };
  if (!rawOrigin) return { valid: true, trustedOrigin, receivedOrigin, secFetchSite, reason: null };
  if (receivedOrigin !== trustedOrigin) return { valid: false, trustedOrigin, receivedOrigin, secFetchSite, reason: 'origin does not match APP_ORIGIN' };
  return { valid: true, trustedOrigin, receivedOrigin, secFetchSite, reason: null };
}
...
module.exports = { configuredOrigin, resolveAppOrigin, validateRequestOrigin, requestOriginDiagnostic, securityHeaders, applySecurityHeaders };
