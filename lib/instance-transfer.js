...
function validateDatabase(filename, Database) {
  ...
  try {
    ...
    return {
      gigs: candidate.prepare('SELECT COUNT(*) AS count FROM gigs').get().count,
      media: candidate.prepare('SELECT COUNT(*) AS count FROM gig_media').get().count
    };
...
function createInstanceTransfer({
  ...
  async function stageImport(request) {
    ...
    try {
      ...
      const summary = validateDatabase(path.join(payloadDir, 'master-list.sqlite'), Database);
      ...
      const marker = {
        format: FORMAT, stagedAt: now().toISOString(), createdAt: manifest.createdAt,
        appVersion: manifest.appVersion, files: payloadFiles, bytes: declaredBytes,
        summary: { ...summary, mediaFiles: [...seen].filter((name) => name.startsWith('media/')).length }
      };
      await fs.writeFile(path.join(stagingDir, 'ready.json'), `${JSON.stringify(marker, null, 2)}\n`, { mode: 0o600 });
      await fs.rename(stagingDir, pendingDir);
      return { staged: true, restartRequired: true, files: payloadFiles, bytes: declaredBytes, createdAt: manifest.createdAt, summary: marker.summary };
    } catch (error) {
...
function applyPendingInstanceImportSync({ fs, path, dataDir, databaseFile, mediaDir, backupDir, connectionsFile, geocodesFile, pendingDir, now = () => new Date(), logger = console }) {
  ...
  function moveDirectoryContentsSync(source, target) {
    fs.mkdirSync(source, { recursive: true, mode: 0o700 });
    fs.mkdirSync(target, { recursive: true, mode: 0o700 });
    for (const name of fs.readdirSync(source)) moveSync(path.join(source, name), path.join(target, name));
  }
  ...
  const targets = [
    ...
    [path.join(dataDir, 'restore-pending.sqlite'), 'restore-pending.sqlite']
  ];
  const installed = []; const installedMedia = [];
  try {
    for (const [target, name] of targets) if (fs.existsSync(target)) moveSync(target, path.join(rollbackDir, name));
    moveDirectoryContentsSync(mediaDir, path.join(rollbackDir, 'media'));
    for (const [name, target] of [['master-list.sqlite', databaseFile], ['connections.json', connectionsFile], ['geocodes.json', geocodesFile]]) {
      const source = path.join(payload, name);
      if (fs.existsSync(source)) { moveSync(source, target); installed.push(target); }
    }
    fs.mkdirSync(mediaDir, { recursive: true, mode: 0o700 });
    for (const name of fs.readdirSync(incomingMedia)) {
      moveSync(path.join(incomingMedia, name), path.join(mediaDir, name)); installedMedia.push(name);
    }
    fs.rmSync(pendingDir, { recursive: true, force: true });
    const result = {
      format: FORMAT, appliedAt: now().toISOString(), sourceCreatedAt: marker.createdAt || null,
      rollbackDirectory: path.relative(dataDir, rollbackDir), summary: marker.summary || null
    };
    ...
    for (const name of installedMedia) fs.rmSync(path.join(mediaDir, name), { recursive: true, force: true });
    for (const [, name] of targets) {
      const source = path.join(rollbackDir, name); const target = targets.find((entry) => entry[1] === name)[0];
      if (fs.existsSync(source) && !fs.existsSync(target)) moveSync(source, target);
    }
    if (fs.existsSync(path.join(rollbackDir, 'media'))) moveDirectoryContentsSync(path.join(rollbackDir, 'media'), mediaDir);
    throw error;
  }
}
