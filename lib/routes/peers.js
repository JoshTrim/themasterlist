function createPeerRoutes({
  ...
  return async function handlePeerRoute(request, response, url) {
    ...
    if (request.method === 'PATCH' && peerMatch) {
      requireAccount(request);
      const body = await readBody(request);
      const name = String(body.name || '').trim();
      if (!name) { sendError(response, 400, 'Peer name is required.'); return true; }
      if (name.length > 100) { sendError(response, 400, 'Peer name must be 100 characters or fewer.'); return true; }
      const result = database.prepare('UPDATE peer_instances SET name = ? WHERE id = ?').run(name, peerMatch[1]);
      if (!result.changes) { sendError(response, 404, 'Paired instance not found.'); return true; }
      sendJson(response, 200, identity.peers().find((peer) => peer.id === peerMatch[1])); return true;
    }
