function createPeerRoutes({
  ...
  function cleanBaseUrl(value) {
    ...
    if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error('Peer URL must be a valid HTTP or HTTPS URL.');
    return baseUrl;
  }
  ...
  async function hello(request, response) {
    try {
      ...
      const timestamp = now();
      database.prepare("UPDATE peer_instances SET status = 'connected', last_seen_at = ? WHERE peer_id = ?").run(timestamp, peer.peer_id);
  ...
  async function importInvite(request, response) {
    ...
    try {
      ...
      if (confirmed) database.prepare("UPDATE peer_instances SET status = 'connected', last_seen_at = ? WHERE peer_id = ?").run(now(), peer.peerId);
  ...
  return async function handlePeerRoute(request, response, url) {
    ...
    if (request.method === 'POST' && url.pathname === '/api/peers/sync-all') {
      requireAccount(request);
      const peers = database.prepare("SELECT * FROM peer_instances WHERE status = 'connected' AND base_url <> '' ORDER BY name COLLATE NOCASE").all();
    ...
    if (request.method === 'POST' && actionMatch) {
      ...
      try {
        if (actionMatch[2] === 'sync') { sendJson(response, 200, await sync.syncWithPeer(peer)); return true; }
