function createMediaMutationRoutes({
  ...
  async function patchMedia(request, response, id) {
    ...
    const artifactTypes = new Set(['merch', 'ticket', 'setlist', 'poster', 'memorabilia']);
    const artifactViews = new Set(['front', 'back', 'detail']);
    const artifactType = 'artifactType' in body ? String(body.artifactType || '').toLowerCase() : null;
    if ('artifactType' in body && (media.category !== 'artifact' || !artifactTypes.has(artifactType))) return sendError(response, 400, 'Choose a valid artifact type.');
    const artifactView = 'artifactView' in body ? String(body.artifactView || '').toLowerCase() : null;
    if ('artifactView' in body && (media.category !== 'artifact' || !artifactViews.has(artifactView))) return sendError(response, 400, 'Choose a valid artifact view.');
    const crop = (field, current, min, max) => {
      if (!(field in body)) return current;
      const value = Number(body[field]);
      return Number.isFinite(value) ? Math.max(min, Math.min(max, value)) : current;
    };
    database.prepare(`UPDATE gig_media SET caption = COALESCE(?, caption), is_cover = COALESCE(?, is_cover),
      ...
      playback_end = CASE WHEN ? THEN ? ELSE playback_end END,
      artifact_type = COALESCE(?, artifact_type), artifact_notes = COALESCE(?, artifact_notes),
      artifact_crop_x = ?, artifact_crop_y = ?, artifact_zoom = ?,
      artifact_view = COALESCE(?, artifact_view), artifact_view_label = COALESCE(?, artifact_view_label) WHERE id = ?`).run(
      ...
      'playbackEnd' in body ? 1 : 0, playbackEnd,
      artifactType, 'artifactNotes' in body ? String(body.artifactNotes || '').trim().slice(0, 2000) : null,
      crop('artifactCropX', media.artifact_crop_x, 0, 100), crop('artifactCropY', media.artifact_crop_y, 0, 100),
      crop('artifactZoom', media.artifact_zoom, 1, 3), artifactView,
      'artifactViewLabel' in body ? String(body.artifactViewLabel || '').trim().slice(0, 80) : null, id
    );
    if (media.category === 'artifact' && media.artifact_group_id) {
      if ('caption' in body || 'artifactType' in body || 'artifactNotes' in body) {
        database.prepare(`UPDATE gig_media SET caption = CASE WHEN ? THEN ? ELSE caption END,
          artifact_type = CASE WHEN ? THEN ? ELSE artifact_type END,
          artifact_notes = CASE WHEN ? THEN ? ELSE artifact_notes END
          WHERE gig_id = ? AND artifact_group_id = ?`).run(
          'caption' in body ? 1 : 0, String(body.caption || '').trim(),
          'artifactType' in body ? 1 : 0, artifactType,
          'artifactNotes' in body ? 1 : 0, String(body.artifactNotes || '').trim().slice(0, 2000),
          media.gig_id, media.artifact_group_id
        );
      }
      if ('artifactIsCover' in body && body.artifactIsCover) {
        database.prepare('UPDATE gig_media SET artifact_is_cover = CASE WHEN id = ? THEN 1 ELSE 0 END WHERE gig_id = ? AND artifact_group_id = ?').run(id, media.gig_id, media.artifact_group_id);
      }
    }
    return sendJson(response, 200, mediaRows(media.gig_id).find((entry) => entry.id === id));
  }

  async function combineArtifact(request, response, targetId) {
    const target = mediaById(targetId);
    if (!target || target.category !== 'artifact') return sendError(response, 404, 'Target artifact not found.');
    const body = await readBody(request);
    const sourceId = String(body.sourceId || '');
    const view = String(body.view || '').toLowerCase();
    const viewLabel = String(body.viewLabel || '').trim().slice(0, 80);
    const source = mediaById(sourceId);
    if (!source || source.category !== 'artifact') return sendError(response, 404, 'Source artifact not found.');
    if (source.gig_id !== target.gig_id) return sendError(response, 400, 'Artifacts can only be combined within the same show.');
    const targetGroupId = target.artifact_group_id || target.id;
    const sourceGroupId = source.artifact_group_id || source.id;
    if (targetGroupId === sourceGroupId) return sendError(response, 400, 'These photos already belong to the same artifact.');
    if (!['front', 'back', 'detail'].includes(view)) return sendError(response, 400, 'Choose front, back or detail for the combined photo.');
    if (view === 'detail' && !viewLabel) return sendError(response, 400, 'Give the detail photo a label.');
    const sourceCount = database.prepare("SELECT COUNT(*) AS count FROM gig_media WHERE gig_id = ? AND category = 'artifact' AND artifact_group_id = ?").get(source.gig_id, sourceGroupId).count;
    if (sourceCount !== 1) return sendError(response, 409, 'Only a standalone artifact can be combined. Move its other views first.');
    if (view !== 'detail') {
      const occupied = database.prepare("SELECT id FROM gig_media WHERE gig_id = ? AND category = 'artifact' AND artifact_group_id = ? AND artifact_view = ?").get(target.gig_id, targetGroupId, view);
      if (occupied) return sendError(response, 409, `This artifact already has a ${view} photo.`);
    }
    const targetIdentity = database.prepare("SELECT caption, artifact_type, artifact_notes FROM gig_media WHERE gig_id = ? AND artifact_group_id = ? ORDER BY artifact_is_cover DESC, sort_order, created_at LIMIT 1").get(target.gig_id, targetGroupId) || target;
    database.transaction(() => {
      database.prepare(`UPDATE gig_media SET artifact_group_id = ?, artifact_view = ?, artifact_view_label = ?, artifact_is_cover = 0,
        caption = ?, artifact_type = ?, artifact_notes = ? WHERE id = ?`).run(
        targetGroupId, view, view === 'detail' ? viewLabel : '', targetIdentity.caption || '', targetIdentity.artifact_type || 'memorabilia', targetIdentity.artifact_notes || '', source.id
      );
    })();
    return sendJson(response, 200, mediaRows(target.gig_id).find((entry) => entry.id === source.id));
  }
  ...
  async function deleteMedia(response, id) {
    ...
    if (media.category === 'artifact' && media.artifact_group_id) {
      const replacement = database.prepare('SELECT id FROM gig_media WHERE gig_id = ? AND artifact_group_id = ? ORDER BY artifact_is_cover DESC, sort_order, created_at LIMIT 1').get(media.gig_id, media.artifact_group_id);
      if (replacement && media.artifact_is_cover) database.prepare('UPDATE gig_media SET artifact_is_cover = 1 WHERE id = ?').run(replacement.id);
    }
  ...
  function startBackgroundRemoval(id) {
    const media = mediaById(id);
    if (!media) throw Object.assign(new Error('Media not found.'), { statusCode: 404 });
    if (media.category !== 'artifact' || !media.mime_type.startsWith('image/')) throw Object.assign(new Error('Background removal is only available for artifact photos.'), { statusCode: 400 });
    if (media.background_status === 'running') throw Object.assign(new Error('Background removal is already running.'), { statusCode: 409 });
    if (!existsSync(path.join(mediaDir, media.filename))) throw Object.assign(new Error('The original artifact photo is missing from disk.'), { statusCode: 409 });
  ...
  function removeBackground(response, id) {
    try { return sendJson(response, 202, { jobId: startBackgroundRemoval(id) }); }
    catch (error) { return sendError(response, error.statusCode || 500, error.message); }
  }
  ...
  const handleMediaMutation = async function handleMediaMutation(request, response, url) {
    ...
    const combineMatch = url.pathname.match(/^\/api\/media\/([\w-]+)\/combine-artifact$/);
    if (request.method === 'POST' && combineMatch) { await combineArtifact(request, response, combineMatch[1]); return true; }
  ...
  handleMediaMutation.startBackgroundRemoval = startBackgroundRemoval;
  return handleMediaMutation;
}
