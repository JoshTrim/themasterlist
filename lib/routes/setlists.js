function createSetlistRoutes({ provider, enrichAlbums, sendJson, sendError }) {
  return async function handleSetlistRoute(request, response, url) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/setlists/event') {
      try {
        const result = await provider.searchEvent({ venueId: url.searchParams.get('venueId')?.trim(), venueName: url.searchParams.get('venueName')?.trim(), cityName: url.searchParams.get('cityName')?.trim(), eventDate: url.searchParams.get('eventDate')?.trim() });
        sendJson(response, 200, result);
      } catch (error) { sendError(response, error.status || 502, error.message); }
      return true;
    }
