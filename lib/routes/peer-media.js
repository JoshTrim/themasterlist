...
function createPeerMediaRoutes({
  ...
  async function copyRemote({ selected, sharedGigId, mediaId, jobId, control }) {
    ...
    let remote; let finalized = false; let persisted = false; let cutoutFilename = null;
    try {
      ...
      if (category === 'artifact' && item.useBackgroundRemoved) {
        const candidateName = `${path.parse(storedFilename).name}.cutout.png`;
        const candidateTemporary = path.join(mediaDir, `${candidateName}.peer-copying`);
        const candidateFinal = path.join(mediaDir, candidateName);
        let cutoutRemote;
        try {
          cutoutRemote = await transport.fetchMedia(peerRow(selected.record), { type: 'peer-media', sharedGigId, mediaId, variant: 'cutout' });
          const output = await fs.open(candidateTemporary, 'wx', 0o600);
          let cutoutBytes = 0;
          try {
            for await (const chunk of cutoutRemote.response.body) {
              const bytes = Buffer.from(chunk); cutoutBytes += bytes.length;
              if (storageUsage() + cutoutBytes > maxStorageSize) throw new Error('The artifact cutout would exceed the configured media storage quota.');
              await output.write(bytes);
            }
          } finally { await output.close(); }
          if (!cutoutBytes || !validMediaSignature(await fileSignature(candidateTemporary), 'image/png')) throw new Error('The peer returned an invalid artifact cutout.');
          await fs.rename(candidateTemporary, candidateFinal);
          cutoutFilename = candidateName;
        } catch {
          await fs.rm(candidateTemporary, { force: true }).catch(() => {});
          await fs.rm(candidateFinal, { force: true }).catch(() => {});
          cutoutFilename = null;
        } finally { cutoutRemote?.abort?.(); }
      }
      ...
      database.transaction(() => {
        ...
        database.prepare(`INSERT INTO gig_media
          (id, gig_id, filename, mime_type, caption, is_cover, sort_order, rotation, category, checksum, size,
            song_index, playback_preferred, playback_start, playback_end, artifact_type, artifact_notes,
            artifact_crop_x, artifact_crop_y, artifact_zoom, artifact_group_id, artifact_view, artifact_view_label, artifact_is_cover,
            background_filename, background_status,
            use_background_removed, created_at)
          VALUES (?, ?, ?, ?, ?, 0,
            ?, ?, ?, ?, ?,
            ?, ?, ?, ?,
            ?, ?,
          ...
          category === 'artifact' ? String(item.artifactType || 'memorabilia') : 'memorabilia',
          category === 'artifact' ? String(item.artifactNotes || '').slice(0, 2000) : '',
          Math.max(0, Math.min(100, Number(item.artifactCropX ?? 50))), Math.max(0, Math.min(100, Number(item.artifactCropY ?? 50))),
          Math.max(1, Math.min(3, Number(item.artifactZoom ?? 1))),
          category === 'artifact' ? String(item.artifactGroupId || item.id || id).slice(0, 100) : null,
          category === 'artifact' && ['front', 'back', 'detail'].includes(item.artifactView) ? item.artifactView : 'front',
          category === 'artifact' ? String(item.artifactViewLabel || '').slice(0, 80) : '',
          category === 'artifact' && item.artifactIsCover ? 1 : 0, cutoutFilename,
          cutoutFilename ? 'complete' : 'not_started', cutoutFilename ? 1 : 0, now()
        );
      ...
      if (cutoutFilename && !persisted) await fs.rm(path.join(mediaDir, cutoutFilename), { force: true }).catch(() => {});
