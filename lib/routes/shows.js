const { normaliseActs } = require('../show-acts');
...
function createShowRoutes({ database, readGigs, readBody, sendJson, sendError, validateGig, normaliseRating, normaliseAttendees, randomUUID, now = () => new Date().toISOString() }) {
  return async function handleShowRoute(request, response, url) {
    ...
    if (request.method === 'POST' && url.pathname === '/api/gigs') {
      ...
      const artist = gig.artist.trim();
      const record = { id: randomUUID(), sharedId: randomUUID(), artist, venue: gig.venue.trim(), city: gig.city.trim(), date: String(gig.date || '').trim(), notes: String(gig.notes || '').trim(), performanceNotes: String(gig.performanceNotes || gig.notes || '').trim(), venueNotes: String(gig.venueNotes || '').trim(), performanceRating: normaliseRating(gig.performanceRating), venueRating: normaliseRating(gig.venueRating), favorite: gig.favorite === true || gig.favorite === 'true', setlistFmId: gig.setlistFmId || null, setlistFmUrl: gig.setlistFmUrl || null, songs: Array.isArray(gig.songs) ? gig.songs : [], acts: normaliseActs(gig.acts).filter((act) => act.artist.toLocaleLowerCase() !== artist.toLocaleLowerCase()), attendees: normaliseAttendees(gig.attendees, request.account), createdAt: now() };
      database.prepare(`INSERT INTO gigs (id, shared_id, artist, venue, city, date, notes, performance_notes, venue_notes, performance_rating, venue_rating, favorite, setlist_fm_id, setlist_fm_url, songs, acts, attendees, created_at)
        VALUES (@id, @sharedId, @artist, @venue, @city, @date, @notes, @performanceNotes, @venueNotes, @performanceRating, @venueRating, @favorite, @setlistFmId, @setlistFmUrl, @songs, @acts, @attendees, @createdAt)`).run({ ...record, performanceRating: record.performanceRating ?? null, venueRating: record.venueRating ?? null, favorite: record.favorite ? 1 : 0, songs: JSON.stringify(record.songs), acts: JSON.stringify(record.acts), attendees: JSON.stringify(record.attendees) });
    ...
    if (request.method === 'PATCH') {
      ...
      if ('acts' in update) gig.acts = normaliseActs(update.acts).filter((act) => act.artist.toLocaleLowerCase() !== gig.artist.toLocaleLowerCase());
      ...
      validateGig(gig);
      database.prepare(`UPDATE gigs SET artist = ?, venue = ?, city = ?, date = ?, songs = ?, acts = ?, attendees = ?, favorite = ?, performance_rating = ?, venue_rating = ?, performance_notes = ?, venue_notes = ?, notes = ? WHERE id = ?`).run(gig.artist, gig.venue, gig.city, gig.date, JSON.stringify(gig.songs || []), JSON.stringify(gig.acts || []), JSON.stringify(gig.attendees || []), gig.favorite ? 1 : 0, gig.performanceRating ?? null, gig.venueRating ?? null, gig.performanceNotes || '', gig.venueNotes || '', gig.notes || '', gig.id);
