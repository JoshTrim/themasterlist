...
function createDirectoryRoutes({
  database, requireAccount, readBody, sendJson, sendError, fetchArtistInfo, refetchArtistInfo, fetchVenueInfo,
  cachedArtistGenres, saveArtistGenres, normaliseImagePosition, profileImages,
  geocoding, validCoordinates, now = () => new Date().toISOString()
}) {
  return async function handleDirectoryRoute(request, response, url) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/directory/artist-images') {
      requireAccount(request);
      const artists = database.prepare('SELECT lookup_name AS lookupName, image, image_position AS imagePosition, updated_at AS updatedAt FROM artist_info').all();
      sendJson(response, 200, artists, { 'Cache-Control': 'private, max-age=60, stale-while-revalidate=300' }); return true;
    }
    ...
    if (request.method === 'POST' && url.pathname === '/api/artists/refetch') {
      requireAccount(request);
      const name = String(url.searchParams.get('name') || '').trim();
      if (!name) { sendError(response, 400, 'An artist name is required.'); return true; }
      try {
        const body = await readBody(request);
        const info = await refetchArtistInfo(name, body.source);
        const existing = database.prepare('SELECT image_position AS imagePosition FROM artist_info WHERE lookup_name = ?').get(name.toLowerCase());
        sendJson(response, 200, {
          ...info,
          imagePosition: normaliseImagePosition(existing?.imagePosition),
          genres: cachedArtistGenres(name)?.genres || []
        });
    ...
    if (request.method === 'PATCH' && url.pathname === '/api/artists') {
      ...
      const uploadedImage = await profileImages.save(body.imageUpload);
      const info = {
