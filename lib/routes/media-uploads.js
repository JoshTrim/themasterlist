function createMediaUploadRoutes({
  ...
  function artifactUploadMeta(request, url, gigId, category, body = null) {
    if (category !== 'artifact') return {};
    const groupId = String(body?.artifactGroupId || request.headers['x-artifact-group-id'] || url.searchParams.get('artifactGroupId') || '').trim();
    const view = String(body?.artifactView || request.headers['x-artifact-view'] || url.searchParams.get('artifactView') || (groupId ? 'detail' : 'front')).toLowerCase();
    const label = String(body?.artifactViewLabel || request.headers['x-artifact-view-label'] || url.searchParams.get('artifactViewLabel') || '').trim().slice(0, 80);
    if (!['front', 'back', 'detail'].includes(view)) throw Object.assign(new Error('Choose front, back or detail for the artifact view.'), { statusCode: 400 });
    let group = null;
    if (groupId) {
      group = database.prepare("SELECT * FROM gig_media WHERE gig_id = ? AND category = 'artifact' AND artifact_group_id = ? ORDER BY artifact_is_cover DESC, sort_order, created_at LIMIT 1").get(gigId, groupId);
      if (!group) throw Object.assign(new Error('The artifact group could not be found for this show.'), { statusCode: 404 });
      const occupied = database.prepare("SELECT id FROM gig_media WHERE gig_id = ? AND artifact_group_id = ? AND artifact_view = ? AND (? <> 'detail' OR artifact_view_label = ?)").get(gigId, groupId, view, view, label);
      if (occupied && view !== 'detail') throw Object.assign(new Error(`This artifact already has a ${view} photo. Remove or relabel it before uploading another.`), { statusCode: 409 });
    }
    return { groupId: groupId || null, view, label, group };
  }

  function insertUpload({ id, gigId, storedFilename, mimeType, caption, category, checksum = null, size, isCover = false, artifact = {} }) {
    ...
    const artifactGroupId = category === 'artifact' ? (artifact.groupId || id) : null;
    const artifactCaption = category === 'artifact' && artifact.group ? artifact.group.caption : caption;
    const artifactType = category === 'artifact' && artifact.group ? artifact.group.artifact_type : 'memorabilia';
    const artifactNotes = category === 'artifact' && artifact.group ? artifact.group.artifact_notes : '';
    const artifactCover = category === 'artifact' && !artifact.groupId ? 1 : 0;
    database.prepare(`INSERT INTO gig_media
      (id, gig_id, filename, mime_type, caption, is_cover, sort_order, rotation, category, checksum, size, created_at,
       artifact_type, artifact_notes, artifact_group_id, artifact_view, artifact_view_label, artifact_is_cover)
      VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
      id, gigId, storedFilename, mimeType, artifactCaption, isCover ? 1 : 0, sortOrder, category, checksum, size, now(),
      artifactType, artifactNotes, artifactGroupId, artifact.view || 'front', artifact.label || '', artifactCover
    );
  ...
  async function handleChunk(request, response, url, match) {
    ...
    if (!session) {
      let artifact;
      try { artifact = artifactUploadMeta(request, url, gigId, category); }
      catch (error) { return sendError(response, error.statusCode || 400, error.message); }
      ...
      session = { gigId, filename, total, category, mimeType, artifact, offset: 0, stored, path: path.join(mediaDir, `${stored}.uploading`) };
    ...
    const id = randomUUID();
    insertUpload({ id, gigId, storedFilename: session.stored, mimeType, caption: filename, category, checksum, size: session.total, artifact: session.artifact });
  ...
  async function handleRawUpload(request, response, url, gigId, category, mimeType) {
    ...
    insertUpload({ id, gigId, storedFilename, mimeType, caption, category, checksum: digest, size, artifact });
  ...
  async function handleJsonUpload(request, response, gigId, category) {
    ...
    if (category === 'artifact' && !acceptedArtifact.test(mimeType)) return sendError(response, 415, 'Artifacts must be uploaded as photos.');
    let artifact;
    try { artifact = artifactUploadMeta(request, new URL('http://local/'), gigId, category, body); }
    catch (error) { return sendError(response, error.statusCode || 400, error.message); }
    ...
    insertUpload({ id, gigId, storedFilename, mimeType, caption: String(body.caption || filename).trim(), category, size: file.length, isCover: body.isCover, artifact });
