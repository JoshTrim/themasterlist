...
function validateAccount(body = {}) {
  ...
  if (!name || name.length > 80) throw validationError('Enter a name up to 80 characters.');
  if (password.length < 10) throw validationError('Use a password with at least 10 characters.');
...
function validateGig(gig = {}) {
  ...
  if (missing.length) throw validationError(`Please provide: ${missing.join(', ')}.`);
}

function normaliseRating(value) {
  if (value === undefined || value === null || value === '') return null;
  const rating = Number(value);
  if (!Number.isInteger(rating) || rating < 1 || rating > 5) throw validationError('Ratings must be whole stars from 1 to 5.');
  return rating;
}
