...
function createFileServing({ fs, legacyFs, path, publicDir, mediaDir, database, profileImages, sendError }) {
  ...
  async function stream(response, filePath, mimeType, rangeHeader, cacheControl) {
    ...
    if (range) {
      response.writeHead(206, { 'Content-Type': mimeType, 'Content-Length': range.end - range.start + 1, 'Content-Range': `bytes ${range.start}-${range.end}/${stat.size}`, 'Accept-Ranges': 'bytes', 'Cache-Control': cacheControl });
      return legacyFs.createReadStream(filePath, range).pipe(response);
    }
    response.writeHead(200, { 'Content-Type': mimeType, 'Content-Length': stat.size, 'Accept-Ranges': 'bytes', 'Cache-Control': cacheControl });
  ...
  async function serveProfileImage(request, response, profileImage) {
    try { return await stream(response, profileImage.filePath, profileImage.mimeType, request.headers.range, 'private, max-age=86400'); }
  ...
  return { serveStatic, handleStoredFile, serveMedia, serveProfileImage };
}
