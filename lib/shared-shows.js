...
function createSharedShows({
  ...
  conflictPayloadFromGig, normaliseRating, randomUUID, now = () => new Date().toISOString()
}) {
  function profiles() {
  ...
  function create(sourceGigId, profileId) {
    ...
    database.prepare(`INSERT INTO shared_shows
      (id, source_gig_id, artist, venue, city, date, setlist_fm_id, setlist_fm_url, songs, acts, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(id) DO UPDATE SET source_gig_id=excluded.source_gig_id, artist=excluded.artist,
        ...
        setlist_fm_url=excluded.setlist_fm_url, songs=excluded.songs, acts=excluded.acts`).run(
      id, sourceGigId, gig.artist, gig.venue, gig.city, gig.date, gig.setlistFmId, gig.setlistFmUrl, JSON.stringify(gig.songs || []), JSON.stringify(gig.acts || []), timestamp
    );
  ...
  function dismiss(showId) {
    const shared = database.prepare('SELECT id, source_gig_id AS sourceGigId FROM shared_shows WHERE id = ?').get(showId);
    if (!shared) throw new Error('Shared show not found.');
    if (shared.sourceGigId && database.prepare('SELECT 1 FROM gigs WHERE id = ?').get(shared.sourceGigId)) {
      throw new Error('This shared record is attached to a local show. Remove the local show or edit its attendees instead.');
    }
    const timestamp = now();
    database.transaction(() => {
      database.prepare(`INSERT INTO dismissed_shared_shows (shared_gig_id, dismissed_at) VALUES (?, ?)
        ON CONFLICT(shared_gig_id) DO UPDATE SET dismissed_at=excluded.dismissed_at`).run(showId, timestamp);
      database.prepare('DELETE FROM notifications WHERE shared_gig_id = ?').run(showId);
      database.prepare('DELETE FROM peer_sync_baselines WHERE shared_gig_id = ?').run(showId);
      database.prepare('DELETE FROM peer_sync_conflicts WHERE shared_gig_id = ?').run(showId);
      database.prepare('DELETE FROM shared_shows WHERE id = ?').run(showId);
    })();
    return { ok: true, id: showId, dismissedAt: timestamp };
  }

  function adopt(showId, account) {
    const shared = database.prepare('SELECT * FROM shared_shows WHERE id = ?').get(showId);
    if (!shared) throw new Error('Shared show not found.');
    const existing = database.prepare('SELECT id FROM gigs WHERE shared_id = ? OR id = ? ORDER BY created_at LIMIT 1').get(showId, shared.source_gig_id || '');
    if (existing) return findGig(existing.id);
    const owner = requireProfile(account?.id);
    const peers = new Map(peerRows().map((peer) => [peer.peerId, peer]));
    const attendees = [{ id: owner.id, type: 'owner', name: owner.name }];
    for (const contribution of contributionRows(showId)) {
      const peer = peers.get(contribution.instanceId);
      if (peer && !attendees.some((attendee) => attendee.id === peer.peerId)) attendees.push({ id: peer.peerId, type: 'peer', name: peer.name });
    }
    const id = randomUUID();
    const timestamp = now();
    let gig;
    database.transaction(() => {
      database.prepare(`INSERT INTO gigs
        (id, shared_id, artist, venue, city, date, notes, performance_notes, venue_notes,
          performance_rating, venue_rating, favorite, setlist_fm_id, setlist_fm_url, songs, acts, attendees, created_at)
        VALUES (?, ?, ?, ?, ?, ?, '', '', '', NULL, NULL, 0, ?, ?, ?, ?, ?, ?)`).run(
        id, showId, shared.artist, shared.venue, shared.city, shared.date, shared.setlist_fm_id, shared.setlist_fm_url,
        shared.songs || '[]', shared.acts || '[]', JSON.stringify(attendees), timestamp
      );
      database.prepare('UPDATE shared_shows SET source_gig_id = ? WHERE id = ?').run(id, showId);
      database.prepare('DELETE FROM dismissed_shared_shows WHERE shared_gig_id = ?').run(showId);
      gig = findGig(id);
      upsertLocalContribution(gig, timestamp);
    })();
    return gig;
  }
  ...
  return { profiles, requireProfile, normaliseAttendees, rows, create, addAttendee, updateReview, dismiss, adopt, applyRemoteMediaAssignments, resolveConflict };
}
